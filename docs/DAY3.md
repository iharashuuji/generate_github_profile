...
Codespace を削除すると、`docker build` で作ったイメージも一緒に消えます。
これは**問題ありません。** `Dockerfile` がリポジトリに残っているので、
次に必要になったとき `docker build` すれば、まったく同じイメージが再び手に入ります。

**環境そのものをコードとして残しておけば、実物はいつでも捨てられる。**
これが、この 3 回を通じて体験してきたことの結論です。
